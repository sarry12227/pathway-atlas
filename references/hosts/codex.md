...
|---|---|---|---|
| search | Callable web/search tools discover candidate public pages. | Pass --host-capability search only when callable now. | Use explicit user-supplied URLs/local artifacts or offline mode. |
| browse | A callable in-app browser or page reader opens public pages and inspects citations; use a purpose-built connector or existing browser state only when task state requires it. | Pass --host-capability browse only when callable now. | Without it, do not claim page verification; request a saved public artifact or use offline mode. |
