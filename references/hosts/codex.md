...
Follow the one-question-at-a-time intake in SKILL.md. Accept each answer as ordinary conversation text, retain the private draft across turns, and show only the current question and its choices. Wait for the reply before advancing. Use the bounded thematic questions in the questionnaire: related details share one question, already answered details are reused, and only decision-relevant gaps get one clarification per theme (six total). Preserve uncollected details separately from explicit unknown or refusal. Use a currently available choice control for one question when it can hold all choices, otherwise show lettered choices. Only the user selects or skips; an unanswered item stays pending.
...
|---|---|---|---|
| search | Callable web/search tools discover candidate public pages. | Pass --host-capability search only when callable now. | With browse, read known official URLs in standard and disclose limited discovery; retain already authenticated material. Without a source reader, use offline mode. |
| browse | A callable in-app browser or page reader opens public pages and inspects citations; use a purpose-built connector or existing browser state only when task state requires it. | Pass --host-capability browse only when callable now. | Try another available page or HTTP reader first. If none works, do not claim page verification; use already authenticated material or offline mode. |
| vision | Callable local image inspection or host vision inspects supplied public images or host-produced OCR and QR text. | Pass --host-capability vision only when callable now. | Seek machine-readable HTML/XLS/XLSX/PDF/text or host-decoded QR payload, or mark the fact missing; the normal boundary remains local/host-native. |
| local_exec | Callable local command execution runs repository CLIs, adapters, and validators. | Workflow gate only; record its state in the handoff. | Without it, stop before deterministic calculation and disclose the limitation. Complete intake and profile confirmation first, then deliver the same seven-part reference using genuinely available evidence. Label uncomputed values unknown, disclose that calculation did not run, retain a resumable handoff, and never emulate a calculated result. |
| file_output | Callable workspace file operations create the evidence workspace and anonymous Markdown or DOCX. | Workflow gate only; record its state in the handoff. | Retain a path-neutral structured handoff internally and explain permitted conclusions in chat; do not claim an evidence bundle or report was written. |
...
Browser and search tools discover or read public content. Retrieved attachments still cross `scripts.downloader` and the matching adapter; QR adapters receive decoded text rather than images. Host vision may inspect already supplied public images locally or in-session. An external OCR or QR service requires explicit user authorization and evidence disclosure; the default is local/host-native processing or a missing fact.
