...
|---|---|---|---|
| search | A callable search API discovers candidate public pages. | Pass --host-capability search only when callable now. | Continue in offline mode with already authenticated material; record discovery unavailable. |
| browse | A callable browser/page reader opens public pages and inspects citations. | Pass --host-capability browse only when callable now. | Without it, do not claim page verification; use already authenticated material or offline mode. |
| vision | Callable vision/OCR inspects supplied public images or host-produced OCR and QR text. | Pass --host-capability vision only when callable now. | Seek machine-readable HTML/XLSX/PDF/text or host-decoded QR payload, or mark the fact missing; the normal boundary remains local/host-native. |
| local_exec | A shell/runner executes repository CLIs, adapters, and validators. | Workflow gate only; record its state in the handoff. | Without it, stop before deterministic calculation, disclose the limitation and move the session to a host with local execution; never emulate a result in prose. |
