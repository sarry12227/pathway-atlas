...
画像确认并运行 `start` 后读取本页。默认目标是先交付可用的升学范围和典型路径，不等待完整研究矩阵。`brief` 与深度 `finish` 使用同一画像，前者输出规划参考，后者保留原有完整证据认证。不要把两种输出状态混称。

## 一轮研究，先形成结论

1. 沿用确认的学校、年级、校排、人数、最好/常态表现、选科、外语、预算和专业。用户已有回答不重问。
2. 优先读本校最近可得出口统计、可比校排锚点和本省最近完整一分一段表；本校不足按往年、明确可比的同城学校回退。搜到可用官方原文即提取，不要求为了凑候选数量再读十个网页。
...
默认首轮以**最多24次原文读取、约8分钟研究**作为交付预算；同一失败动作最多重试一次，某个工具确定不可用后本轮换路线，不在每个任务上重复失败。达到预算或定位与代表校足够时，立即交付已有结果；候选缺额写出数量和原因，不为填满名单无限补洞。未研究的深度任务保持pending，不批量伪报unavailable。用户明确要求更深核验时，再按原研究流程继续。
...
- 有校排与学校出口锚点：按相邻锚点线性插值，得到省排中心；用最好/常态校排与明确的规划浮动形成范围，再按一分一段反查高考分数。先写“本次XX分，按YYYY年口径对应高考大致XX分、省排约XX位”，再用一两句交代依据和范围。高一、高二也做这一步，不以“还没高考”为由放弃。
- `planning-reference-v1` 的默认位次浮动为20%，可在10%–50%内根据年级、可比性和波动明确调整；这是敏感性假设，**不是经回测的置信区间**，不制造自动提分。原文中的校排锚点保持原样，当前校排不能超出已有锚点范围后无声外推。
...
| `calibration` | 见下方三种输入；不直接填一个自编的省排中心 |
...
校排校准为`{method: "school_rank", school, year, comparability: "same_school", basis, margin_fraction: 0.2, anchors: [{school_rank, province_rank, citation}, ...]}`。至少两个同口径相邻锚点；school必须与画像高中一致。同城代理使用`regional_comparable`，补`comparability_reason`说明客观可比依据，正文披露为代理估算。正式高考位次为`{method: "official_rank", basis}`，只从画像读取实际位次；有当年正式高考分数但无位次用`official_score`，要求当年表含该分数的精确行。无锚点为`{method: "unavailable", basis}`；这不妨碍生成其余版块。
