...
```bash
请使用你当前环境的 Skill 安装能力，从 GitHub `https://github.com/sarry12227/pathway-atlas` 安装或更新 `pathway-atlas`（多元星途）到最新版；如果 GitHub 无法访问，请改用 Gitee 镜像 `https://gitee.com/sarry1/pathway-atlas`。若没有专用安装工具，请将完整仓库克隆或下载到当前 Agent 可识别的 Skills 目录，保留已有咨询记录，确认根目录存在 `SKILL.md` 且其中 `name` 为 `pathway-atlas`，再按当前环境支持的方式重新加载并调用它。若无法安装或加载，请如实说明具体阻碍和最少下一步，不要声称已调用。
```
...
| 你关心的问题 | 规划会给你的帮助 |
| --- | --- |
| **现在的水平，该怎样看学校？** | 先给本次分数、高考参考分数与省排中心及范围，再按意向专业列本省冲3所、稳4所、保5所典型学校 |
| **哪些升学路径值得投入？** | 强基、综评比较面向生源省的全国学校，港澳同档香港优先；先给冲1/稳1/保1及同档备选，再讲分数为什么值得考虑、怎样培养和需要补足什么 |
| **哪些限制会影响决定？** | 核对选科、外语、费用、体检及服务义务等因素，说明哪些已明确、哪些还需要查证 |
| **现在先做什么？** | 最后给1–5项现阶段优先行动，写清何时做什么、完成标准和复盘节点 |

**结论直接写在对话里。** 你会读到总体判断、院校与专业分析、路径比较、分阶段计划和风险依据；有材料支持的数字和政策附对应来源。Markdown 文件作为补充下载，环境支持时可提供 Word（DOCX），用户无需先打开文件才能理解规划。

默认先做一轮有界研究，交付固定七部分参考规划，不等待几十项深度任务全部结束。学校或联考给了划线表？直接发图，或跟着 AI 一项项说出线值，就能用可比划线的线差／插值估算高考参考分数，再查对应省排；当前 AI 读不了图时会改用简短文字问答。没有划线也可以：学校发布、第三方文章和抖音喜报中的分数段人数、上线率、层次人数，都可作为注明出处的校排粗估依据。不要求喜报自带“校排—省排表”，也不要求第三方必须凑满三个来源。暂缺有效校准资料时，仍列有来源的学校目标梯度和路径。已知选科不符的专业不会为凑数列入，估算与保档都不是录取保证。
...
| 综合评价／三位一体 | 结合高考成绩及其他考核信息进行评价的招生方式 | 全国哪些项目面向考生所在省招生，各校怎样评价，需要准备哪些材料 |
...
可以，这正是提前规划的用途。Agent会查本校近届出口和校排锚点，估算高考参考分数及位次，再给典型学校与路径；缺少个人校准时先交付真实学校的目标梯度，清楚说明条件。无需等到高三才讨论学校和培养方向。
...
[GitHub 主源](https://github.com/sarry12227/pathway-atlas)与[Gitee 镜像](https://gitee.com/sarry1/pathway-atlas)同步维护。当前为 **v0.2.7 公开预览**；第三方目录可能有缓存，已收录不等于已更新，逐项状态见[分发记录](DISTRIBUTION.md)。
...
默认参考把可读来源、喜报观察与推算依据保存在私有记录中，再生成固定正文；单份第三方喜报也能用于披露假设的估算。用户要求深度认证时，才进一步把精确事实写入证据包并通过完整门禁。故障处理见[研究恢复指南](references/research-recovery.md)。
...
3. **有界研究**：`query_plan.py` 从已确认画像与省份配置生成 canonical QueryPlan。当前查询年按会话日期确定，各数据族按 `Y → Y-1 → Y-2 → Y-3` 回查。首轮优先获取喜报人数、一分一段、学校和路径材料，约8分钟或24次原文读取后形成规划，不等完整矩阵。
4. **来源与估算**：Agent保存真实网页、附件或已读取的图片/视频文字，将喜报累计人数或上线率转换为校位边界，用同年省表推算位次。第三方转述、代理与模型假设明确标注，不要求家长提供学校内部换算表。
5. **计算与报告生成**：默认执行 `brief`，程序计算参考位置、筛选学校并生成七部分正文；专业、地域、预算与风险偏好参与判断。用户明确要求深入核验时，才继续 `next → ingest → finish` 完整证据流程；其门禁不阻塞默认参考交付。
6. **对话结论与行动**：Agent直接发出成绩定位、本省冲3稳4保5、三类重点路径各冲1稳1保1、其他路径及1–5项“当前最需要做的事”，按时间与价值排序。来源、证据状态、覆盖范围和不确定性在影响决策处说明；已知限制保留，真实缺额说明。Markdown或可选DOCX作为附件，不能替代聊天正文。当前为公开预览，规划不构成录取承诺。
