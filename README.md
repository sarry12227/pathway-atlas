...
默认先做一轮有界研究，交付固定七部分参考规划，不等待几十项深度任务全部结束。校考通过本校历史出口与校排锚点校准；暂缺校准资料时，仍列有来源的学校目标梯度和路径，说明条件。数量不足的档位如实标缺额，已知选科不符的专业不会为凑数列入。估算与保档都不是录取保证。
...
[GitHub 主源](https://github.com/sarry12227/pathway-atlas)与[Gitee 镜像](https://gitee.com/sarry1/pathway-atlas)同步维护。当前为 **v0.2.0 公开预览**；第三方目录可能有缓存，已收录不等于已更新，逐项状态见[分发记录](DISTRIBUTION.md)。
