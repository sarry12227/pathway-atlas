...
默认先做一轮有界研究，交付固定七部分参考规划，不等待几十项深度任务全部结束。校考定位主要结合网上高考喜报：学校发布、第三方文章和抖音分享中的分数段人数、上线率、层次人数，都可作为注明出处的粗估依据，再通过一分一段表换算省排与参考分数。不要求喜报自带“校排—省排表”，也不要求第三方必须凑满三个来源。暂缺有效人数资料时，仍列有来源的学校目标梯度和路径。已知选科不符的专业不会为凑数列入，估算与保档都不是录取保证。
