...
2. **像聊天一样，一题一题回答。** 第一句话直接进入孩子的情况，例如“孩子在哪个省份参加高考？”。Agent 沿用已有答案，每轮只问一道题、展示当前选项，等你回答后继续；必要准备在等待回答时处理，无需再选择是否开始。暂时不清楚可以说“不确定”，说错了可以随时修改；最后由你确认汇总的个人情况。
3. **阅读结论，按优先级行动。** 确认后 Agent 自动开始查证资料，在对话里解释完整规划。后续有新的考试、目标或限制时，在保留原记录的会话里补充变化，更新受影响的判断。
...
**开始前需要先配置工具、看更新日志或验收流程吗？**

不需要。先聊孩子的情况，Agent 利用等待回答的时间处理必要准备；只在确实需要你操作时说明最少下一步。已经回答的信息继续保留，准备完成后直接衔接，不再让你说一次“开跑”。
...
下面是宿主接入与开发维护资料，按实际需要查阅。安装、测试或本地封装适配可在等待回答时静默处理，不是每次咨询必跑的清单；学生和家长直接按“三步开始”发起咨询。
