...
main 的 `0.1.6` 统一采用暖色书页与多条星途 Logo，保留 `0.1.5` 的新手介绍与 `0.1.4` 的取证恢复、XLS/PDF适配和准备版交付。完整介绍以 README 为准，部分平台展示 SKILL.md 的简介与正文，两处均同步维护。下表逐行记录各平台最近一次单独核验的版本；条目已收录不表示缓存已刷新。腾讯后续更新包使用平台版本 `1.0.7`，对应主源 `0.1.6`，需在已有条目中更新，独立技能图标也需单独上传。
