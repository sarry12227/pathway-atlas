...
依次运行 `python scripts/validate_data.py` 和 `python scripts/validate_evidence.py`。形成 authenticated snapshot 之前不得给出数字或开始计算；官方证据不足时保留 `partial`、`conflict` 或 `missing`，不降低门槛。
