...
PROMPT = (
    ...
    "调用后从我这条消息自动回填已知答案，完成不超过 20 题的匿名规划问卷并让我确认匿名画像；"
    "不要索取姓名、电话、身份证、住址或本地文件路径。请基于可验证的公开来源，先验证证据再计算；"
    "没有官方位次时用可靠的学校、班型、考试与历史锚点形成乐观、中性、保守位次，为我给出普通批冲稳保院校范围，"
    "并对每条多元升学路径明确判断为主攻、重点准备、备选、观察或不建议，同时标注来源、证据状态、覆盖范围和不确定性。」"
)
