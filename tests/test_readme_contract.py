...
```bash
请使用多元星途帮我做升学规划。先沿用我已经提供的信息，直接问当前最相关的一道题；没有学生信息时就问“孩子在哪个省份参加高考？”，有选项时只展示当前题的选项，等待我回答。同时使用当前环境的 Skill 安装能力，从 GitHub `https://github.com/sarry12227/pathway-atlas` 安装或更新 `pathway-atlas`（多元星途）；GitHub 无法访问时改用 Gitee 镜像 `https://gitee.com/sarry1/pathway-atlas`。没有专用安装工具时，将完整仓库克隆或下载到当前 Agent 可识别的 Skills 目录，保留已有咨询记录，确认根目录存在 `SKILL.md` 且 `name` 为 `pathway-atlas`，再按环境支持的方式加载。利用等待我回答的时间，按需静默完成加载、调试、测试及已获授权的本地维护；只在确实需要我操作时说明最少下一步，不播报技术日志，不再让我选择是否开跑。不支持后台执行时先问问题，再分轮处理必要准备，不假称后台运行或已经加载。个人情况收齐后请我确认，随后自动检索并在对话中给出七部分规划：本次分数、高考参考分数与省排；本省冲3稳4保5；强基、综评、港澳各冲1稳1保1及培养介绍；其他可行路径；现阶段行动。说明估算依据与限制，文件作为补充。
```
"""
INTRODUCTION = (
